...
export class AuthService {
  ...
  async signUp(createUserDto: CreateUserDto): Promise<SignupEntity> {
    ...
    const url = `http://getrestyle.com/auth/verify-email?accessToken=${tokens.accessToken}`;
    return {
  ...
  async forgotpassword(email: string): Promise<ForgotpasswordEntity> {
    ...
    if (user.verifyemail == true) {
      const url = `http://getrestyle.com/auth/reset-password?accessToken=${tokens.accessToken}`;
      return {
